package constant;

public class Constants {
    public static final String READ_CAR_NAMES = "경주할 자동차 이름을 입력하세요(이름은 쉼표(,)를 기준으로 구분).";
    public static final String READ_NUMBER_OF_TRY = "시도할 회수는 몇회인가요?";
    public static final String PRINT_WINNER_LIST_GUIDE = "가 최종 우승했습니다.";
    public static final String CAR_NAME_LIMIT_EXCEPTION = "5글자 이하로 입력하세요.";
    public static final String NOT_NUMERIC_EXCEPTION = "숫자만 입력할 수 있습니다.";
    public static final String TRIAL_INPUT_EXCEPTION = "1회 이상만 입력할 수 있습니다.";
    public static final String COMMA_NOT_ALLOWED_EXCEPTION = ",는 하나만 입력해주세요.";
    public static final String MINIMUM_NUMBER_OF_CARS_EXCEPTION = "자동차 이름은 최소 2개 이상 입력해야 합니다.";
    public static final String GAME_STATUS_DELIMITER = " : ";
    public static final String NAME_JOINING_DELIMITER = ", ";
    public static final String POSITION_PRINT_FORMAT = "-";
    public static final String EXCEPTION_STARTER = "[ERROR] ";
}
