...
public class NameInputValidator {
	...
	private static final String HAS_DUPLICATE_NAME_ERROR_MESSAGE = "[error] 입력한 자동차 이름 중 중복되는 이름이 있습니다.";
	...
	public static String validateCarNames(final String carNames) {
		...
		if (!hasBlankInNames(names) && !hasDuplicateNames(names) && isValidNamesLength(names)) {
			return carNames;
		}
	...
	private static boolean hasDuplicateNames(final List<String> names) {
		int distinctNameCount = (int)names.stream().distinct().count();
