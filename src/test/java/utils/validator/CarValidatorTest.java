...
class CarValidatorTest {
    ...
    class CarNameTest {
        ...
        void validateNotDuplicatedCarName() {
            assertThatNoException()
                    .isThrownBy(() -> new CarValidator().validate("호기,재즈,상돌,아루"));
        }

        @Test
        @DisplayName("중복된 자동차 이름이 존재하면 에러를 발생시킨다.")
        void validateDuplicatedCarName() {
            Assertions.assertThatThrownBy(() -> new CarValidator().validate("호기,재즈,상돌,아루,상돌"))
    ...
    class NumberOfCarsSizeTest {
        ...
        void validateNumberOfCarsIsLess2() {
            Assertions.assertThatThrownBy(() -> new CarValidator().validate("호기"))
                    .isInstanceOf(IllegalArgumentException.class)
