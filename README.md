...
1. 주어진 횟수 동안 n대의 자동차는 전진할 수 있다. 
2. 주어진 횟소 동안 n대의 자동차는 멈출 수 있다.
3. 각 자동차에 이름을 부여할 수 있다. 
4. 자동차의 이름은 쉼표(,)를 기준으로 구분한다. 
5. 전진하는 자동차를 출력할 수 있다. 자동차를 출력할 때, 자동차 이름을 같이 출력한다.
6. 사용자는 몇 번의 이동을 할 것인지 입력할 수 있어야 한다.
7. 0에서 9사이에서 random 값을 구한다.
8. random값이 4이상일 경우 전진한다. 
9. random값이 3이하일 경우 멈춘다.
10. 자동차 경주 게임을 완료한 후 누가 우승했는지를 알려준다. 우승자는 한 명 이상일 수 있다.
