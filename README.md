...
# 🧐 추가 반영할 부분
- test에서 getter를 지양해야한다면? -> extracting을 다음에 대체해서 사용해볼까?
- [x] 입력을 받아서 입력 받은 DTO를 컨트롤러에 객체를 생성해서 넘겨주느냐, 아니면 각 객체에서 DTO를 자체적으로 변환하느냐 (CarNameRequst를 Cars로 변환하려는 시도)
- [x] 재입력 처리의 책임이 컨트롤러가 맞을지, 예외를 처리하는 클래스가 있어야할지.
- [x] 가능한 class를 record로 바꿨을 때 부작용이 있을지?? : 모든 변수가 private final이어야한다는 점이라 상관없음
